...
def balanced(m):
    ...
    total_l = length(left(m)) * total_mass(left_m)
    total_r = length(right(m)) * total_mass(right_m)
    return total_l == total_r and balanced(left_m) and balanced(right_m)
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        return label(t)
    for b in branches(t):
        path_sum = label(t) + max_path_sum(b)
        max_sum += [path_sum]
    return max(max_sum)    
