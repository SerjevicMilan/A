...
def accuracy(typed, source):
    ...
    if not src_len and not typed_len:
        return 100.0
    if src_len and not typed_len:
        return 0.0

    for i in range(0,min(src_len,typed_len)):
        if source_words[i] == typed_words[i]:
            corect_words += 1
    return  (corect_words / typed_len) * 100
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        if typed in cache:
            ...
            cache[typed] = diff_function(typed, source, limit)
            return cache[typed]
        # END PROBLEM EC
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    'butter'
    >>> autocorrect("hwllo", ["butter", "hello", "potato"], ten_diff, 9)
    'hwllo'
    ...
    for l_word in word_list:
        if l_word == typed_word:
            return typed_word

    min_dif = min(word_list, key = key)

    if diff_function(typed_word, min_dif, limit) <= limit:
        return min_dif
    return typed_word
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    assert type(typed) == str
    assert type(source) == str
    assert type(limit) == int
    assert limit >= 0

    def f(typed, source, lim):
        if not len(typed):
            return len(source)
        if not len(source):
            return len(type)
        if not lim:
            return limit + 1
        if typed[0] == source[0]:
            return 0 + f(typed[1:], source[1:], lim)
        else:
            return 1 + f(typed[1:], source[1:], lim - 1)
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed[0] == source[0]:
        ...
        remove = typed[1:]
        substitute = source[0] + remove
        if len(source) > 1 and  len(typed) > 1 and typed[1] == source[1]:
            return 1 + minimum_mewtations(substitute[1:], source[1:], limit - 1)
        if len(typed) > 1 and typed[1] == source[0]:
            return 1 + minimum_mewtations(remove[1:], source[1:],limit - 1)
        return 1 + minimum_mewtations(add[1:], source[1:], limit - 1)
...
def final_diff(typed, source, limit):
    ...
    if typed[0] == source[0]:
        ...
        substitute = source[0] + remove
        if len(source) > 1:
            swap = source[1] + source[0] + typed[2:]
        if len(source) > 1 and  len(typed) > 1 and typed[0] == source[1] and typed[1] == source[0]:
            return 1 + minimum_mewtations(swap[2:], source[2:], limit - 1)
        if len(source) > 1 and  len(typed) > 1 and typed[0] == source[1]:
            return 1 + minimum_mewtations(add[1:], source[1:], limit - 1)
        if len(source) > 1 and  len(typed) > 1 and typed[1] == source[1] and source[0] == source[1]:
            return 1 + minimum_mewtations(add[1:], source[1:], limit - 1)
        if len(source) > 1 and  len(typed) > 1 and typed[1] == source[1]:
            return 1 + minimum_mewtations(substitute[1:], source[1:], limit - 1)
        if len(typed) > 1 and typed[1] == source[0]:
...
def report_progress(typed, source, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    def n_corect_words():
        """it cheks for first mismatching word and returns its position in array"""
        t_len = len(typed)
    ...
    progress = n_corect_words() / len(source)
...
def time_per_word(words, timestamps_per_player):
    ...
    times, prev_time = [], 0
    times = [[] for p in tpp]
    for player in range(0, len(tpp)):
        times[player] = [tpp[player][i + 1] - tpp[player][i]  for i in range(0, len(tpp[player]) - 1)]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for w in word_indices:
        ...
        for p in player_indices:
            if words_and_times['times'][fastest][w] > words_and_times['times'][p][w]:
                fastest = p
        result[fastest] = result[fastest] + [words_and_times['words'][w]]
    return result
