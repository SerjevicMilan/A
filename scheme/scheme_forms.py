...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        do_lambda_form(Pair(expressions.first.rest,expressions.rest), env))
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        names, names.rest = Pair(bindings.first.first, nil), names
        vals, vals.rest = Pair(scheme_eval(bindings.first.rest.first, env), nil), vals
...
def do_enumerate_form(expressions, env):
    validate_form(expressions, 1)
    if expressions.first.first != 'quote':
        raise SchemeError('quote missing')
    expressions = expressions.first.rest.first
    if not expressions:
        return nil
    i = 0
    enum = Pair(Pair(i, Pair(expressions.first, nil)), nil)
    list_p = enum
    expressions = expressions.rest
    while expressions:
        i += 1
        enum.rest = Pair(Pair(i, Pair(expressions.first, nil)), nil)
        enum = enum.rest
        expressions = expressions.rest
    return list_p

def do_merge_form(expressions, env):
    validate_form(expressions.rest, 2)
    sign = expressions.first
    exp1 = scheme_eval(expressions.rest.first, env)
    exp2 = scheme_eval(expressions.rest.rest.first, env)
    if not exp1:
        return exp2
    if not exp2:
        return exp1
    list = []
    while exp1:
        list.append(exp1.first)
        exp1 = exp1.rest
    while exp2:
        list.append(exp2.first)
        exp2 = exp2.rest
    list.sort()
    if sign == '<':
        list.reverse()
    sorted = Pair(list[0], nil)
    for k in range(1,len(list)):
        sorted= Pair(list[k],sorted)
    return sorted
