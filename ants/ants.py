...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if self.damage > 0 and not self.double:
            self.damage *= 2
            self.double = True
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        cur_palace = self.place
        pos = 0
        while not cur_palace.is_hive and pos <= self.upper_bound:
            if cur_palace.bees and pos >= self.lower_bound:
                return random_bee(cur_palace.bees)
            pos += 1
            cur_palace = cur_palace.entrance
        return None
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chew_cooldown:
            self.chew_cooldown -= 1
        else:
            bees = self.place.bees
            if bees:
                bee = random_bee(bees)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        palace = self.place.exit
        while palace:
            if palace.ant:
                if palace.ant.is_container and palace.ant.ant_contained:
                    Ant.double(palace.ant.ant_contained)
                else:
                    Ant.double(palace.ant)
            palace = palace.exit
