...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        ...
        if operator == 'define': # define expressions
            return eval_define(exp.rest)
        operands = exp.rest.map(calc_eval)
        if operator == 'and': # and expressions
            return eval_and(operands)
        else: # Call expressions
            if operator in OPERATORS:
                return calc_apply(OPERATORS[operator],operands)
            return calc_apply(OPERATORS[bindings[operator]],operands)
...
def floor_div(args):
    ...
    def f(res, divisors):
        if not divisors:
            ...
            return f(res // divisors.first, divisors.rest)
    return f(args.first, args.rest)
...
def eval_and(expressions):
    ...
    1
    >>> eval_and(Pair(False, Pair("1", nil)))
    False
